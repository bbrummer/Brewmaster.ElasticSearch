								...
								new ScriptResource
                                        {
                                            ...
                                            Credential = "vmadmin",
											TestScript =
													@"if (Test-Path -LiteralPath ""C:\setup\elasticsearch-1.7.0.zip"" -PathType Leaf)
{Write-Verbose ""C:\setup\elasticsearch-1.7.0.zip already exists."" -Verbose
return $true}
return $false",
											SetScript =
													@"Invoke-WebRequest 'https://download.elastic.co/elasticsearch/elasticsearch/elasticsearch-1.7.0.zip' -OutFile ""C:\setup\elasticsearch-1.7.0.zip"""
											,
											GetScript =
													@"return @{ Downloaded = Test-Path -LiteralPath ""C:\setup\elasticsearch-1.7.0.zip"" -PathType Leaf }",
											Requires = new[] {"[File]SetupFolder"}
										},
								new ScriptResource
                                        {
								...
								new GenericResource("Archive")
										{
											...
											Args = new Dictionary<string, string>
                                                {
													{"Path" , @"C:\setup\elasticsearch-1.7.0.zip"},
													{"Destination" , @"%ProgramFiles%"},
													{"Ensure" , "Present"}
												},
...
return $false",
											SetScript =
													@"$servicebat = ""$env:ProgramFiles\elasticsearch-1.7.0\bin\service.bat""
								...
								new ScriptResource
                                        {
                                            ...
                                            Credential = "vmadmin",
											TestScript =
													@"if (Test-Path -LiteralPath ""$env:ProgramFiles\elasticsearch-1.7.0\plugins\head"" -PathType Container)
{Write-Verbose ""Elastic Search Head Plugin already installed."" -Verbose
return $true}
return $false",
											SetScript =
													@"$pluginbat = ""$env:ProgramFiles\elasticsearch-1.7.0\bin\plugin.bat""
...
Start-Process -FilePath $pluginbat -ArgumentList $pluginbatargs -UseNewEnvironment -Wait -RedirectStandardOutput $env:BrewmasterDir\Logs\headpluginlog.log",
											GetScript =
													@"return @{ Installed = Test-Path -LiteralPath ""$env:ProgramFiles\elasticsearch-1.7.0\plugins\head"" -PathType Container }",
											Requires = new[] {"[Script]InstallElasticSearchService"}
										},
								new ScriptResource
                                        {
                                            ...
                                            Credential = "vmadmin",
											TestScript =
													@"if (Test-Path -LiteralPath ""$env:ProgramFiles\elasticsearch-1.7.0\plugins\cloud-azure"" -PathType Container)
...
Start-Process -FilePath $pluginbat -ArgumentList $pluginbatargs -UseNewEnvironment -LoadUserProfile -Wait -RedirectStandardOutput $env:BrewmasterDir\Logs\azurepluginlog.log",
											GetScript =
													@"return @{ Installed = Test-Path -LiteralPath ""$env:ProgramFiles\elasticsearch-1.7.0\plugins\cloud-azure"" -PathType Container }",
											Requires = new[] {"[Script]InstallElasticSearchService"}
										},
								new ScriptResource
                                        {
                                            ...
                                            Credential = "vmadmin",
											TestScript =
													@"if (Select-String -path ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" -pattern ""cloud:"" -allmatches -simplematch -quiet)
{Write-Verbose ""Elastic Search Config already has Cloud settings"" -Verbose
return $true}
return $false",
											SetScript =
													@"Add-Content ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" ""`ncloud:`n`tazure:`n`t`tkeystore: C:/Setup/azurecert.pfx`n`t`tpassword: {{AzureCertificatePassword}}`n`t`tsubscription_id: {{AzureSubscriptionId}}`n`t`tservice_name: {{CloudService}}`n`t""",
											GetScript =
													@"return @{ Configured = Select-String -path ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" -pattern ""cloud:"" -allmatches -simplematch -quiet }",
								...
								new ScriptResource
                                        {
                                            ...
                                            Credential = "vmadmin",
											TestScript =
													@"if (Select-String -path ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" -pattern ""discovery:"" -allmatches -simplematch -quiet)
{Write-Verbose ""Elastic Search Config already has Discovery settings"" -Verbose
return $true}
return $false",
											SetScript =
													@"Add-Content ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" ""`ndiscovery:`n`t`ttype: azure""",
											GetScript =
													@"return @{ Configured = Select-String -path ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" -pattern ""discovery:"" -allmatches -simplematch -quiet }",
											Requires = new[] {"[Script]InstallPluginAzure"}
										},
								new ScriptResource
                                        {
                                            ...
                                            Credential = "vmadmin",
											TestScript =
													@"if (Select-String -path ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" -pattern ""path.data: F:/"" -allmatches -simplematch -quiet)
{Write-Verbose ""Elastic Search Config already has Discovery settings"" -Verbose
return $true}
return $false",
											SetScript =
													@"Add-Content ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" ""`npath.data: F:/""",
											GetScript =
													@"return @{ Configured = Select-String -path ""$env:ProgramFiles\elasticsearch-1.7.0\config\elasticsearch.yml"" -pattern ""path.data: F:/"" -allmatches -simplematch -quiet }",
